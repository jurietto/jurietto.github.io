document.addEventListener("DOMContentLoaded", () => {
  fetch("sketchbook.json")
    .then(res => {
      if (!res.ok) throw new Error(`Failed to fetch sketchbook.json: ${res.status}`);
      return res.json();
    })
    .then(data => {
      ...
      const container = document.getElementById("sketchbook-list");
      const tagList = document.getElementById("tag-list");

      // Render initial
      renderTags(allEntries);
      renderSketchbook(filteredEntries);
      ...
      // Entries renderer
      function renderSketchbook(entries) {
        container.innerHTML = "";
        entries.forEach(e => {
          if (!e.path) return;

          const entry = document.createElement("div");
          entry.className = "sketchbook-entry list-item grid-view-item";
          ...
          meta.className = "sketchbook-meta";
